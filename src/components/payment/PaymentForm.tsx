...
const paymentOptions: {
...
}[] = [
  {
    id: 'kbzpay',
    name: 'KBZPay',
    desc: 'KBZ Bank mobile wallet',
    accent: 'border-blue-500 ring-blue-500/20',
  },
  {
    id: 'cbpay',
    name: 'CB Pay',
    desc: 'CB Bank mobile payment',
    accent: 'border-violet-500 ring-violet-500/20',
  },
  {
    id: 'mmqr',
    name: 'MMQR',
    desc: 'Myanmar national QR standard',
    accent: 'border-emerald-500 ring-emerald-500/20',
  },
]
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-3">
      <div className="space-y-4 lg:col-span-2">
        <PageHeader
          title="Checkout"
          description="Pay via a local gateway to confirm your booking"
          backLabel="Back to details"
          onBack={() => !busy && goToStep('passengers')}
        />

        <PaymentCountdown
          secondsLeft={reservationSecondsLeft}
          ...
          <div className="ui-card overflow-hidden">
            <div className="border-b border-slate-100 bg-slate-50 px-5 py-3">
              <p className="text-sm font-medium text-slate-800">
                Scan to pay with {paymentLabel(paymentMethod)}
              </p>
              <p className="text-xs text-slate-500">
                Mock QR · Amount {formatPrice(totalPrice)}
              </p>
            </div>
            <div className="flex flex-col items-center gap-4 px-5 py-6">
              <MockQRCode data={qrPayload} />
              <p className="text-center text-xs text-slate-500">
                Open your {paymentLabel(paymentMethod)} app and scan this code
              </p>
              <button
                type="button"
                disabled={busy}
